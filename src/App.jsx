...
import { Radio, Waves, Map, MessageCircle, UserRound, Sparkles, Search, Bell, Navigation, Users, Heart, Send, LogIn, LogOut, Plus, X, RefreshCw, Image, Video, FileText } from 'lucide-react'
import { getCurrentUser, ensureUserDocument, getUserProfile, updateUserProfile, listMoments, listUsers, listNotifications, getMoment, listUserMoments, createMoment, joinMoment, reactToMoment, addComment, listComments, followUser, unfollowUser, isFollowing, listActivity, login, register, logout, uploadMedia } from './services/appwrite'
...
export default function App() {
  ...
  const [searchOpen,setSearchOpen]=useState(false), [notificationsOpen,setNotificationsOpen]=useState(false), [refreshing,setRefreshing]=useState(false), [profiles,setProfiles]=useState({})
  ...
  const load = async () => {
    ...
    try {
      ...
      const ms=await listMoments(); setMoments(ms); const ids=[...new Set(ms.map(m=>m.ownerId).filter(Boolean))]; const ps={}; await Promise.all(ids.map(async id=>{try{ps[id]=await getUserProfile(id)}catch{}})); setProfiles(ps)
    } catch (e) {
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <div className="top-actions"><button className="icon-btn" onClick={refresh} title="Refresh"><RefreshCw className={refreshing?'spin':''} size={18}/></button><button className="icon-btn" title="Search" onClick={()=>setSearchOpen(true)}><Search size={19}/></button><button className="icon-btn" title="Notifications" onClick={()=>setNotificationsOpen(true)}><Bell size={19}/></button></div>
    </header>
    ...
    {searchOpen && <SearchPanel moments={moments} profiles={profiles} onClose={()=>setSearchOpen(false)} />}{notificationsOpen && <NotificationsPanel user={user} onClose={()=>setNotificationsOpen(false)} />}
...
function SearchPanel({moments,profiles,onClose}){const [q,setQ]=useState(''); const [users,setUsers]=useState([]); useEffect(()=>{if(q.trim())listUsers(q).then(setUsers).catch(()=>setUsers([]));else setUsers([])},[q]); const results=moments.filter(m=>(m.title+' '+(m.description||'')+' '+(m.category||'')).toLowerCase().includes(q.toLowerCase())).slice(0,12);return <div className="modal-backdrop" onClick={onClose}><div className="sheet search-sheet" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}><X size={19}/></button><div className="eyebrow">DISCOVER</div><h2>Search WORLD</h2><input autoFocus className="field" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search Moments, categories…"/>{q&&results.length?results.map(m=><div className="search-result" key={m.$id}><b>{m.title}</b><small>{profiles[m.ownerId]?.displayName||'WORLD User'} · {m.category} · {timeAgo(m.$createdAt)}</small></div>):q&&users.length?users.map(u=><div className="search-result" key={u.$id}><b>{u.displayName||u.username}</b><small>@{u.username}</small></div>):q?<div className="empty compact"><p>No matching Moments.</p></div>:<div className="empty compact"><p>Type to search the live layer.</p></div>}</div></div>}
...
function Now({moments, profiles, loading, onJoin, onReact, onComment, onCreate}) {
  return <div className="page"><section className="hero"><div className="hero-copy"><span className="live-pill"><i/> HAPPENING NOW</span><h1>Be where<br/><em>life is.</em></h1><p>Discover what is happening around you — or anywhere in the WORLD.</p><button className="primary" onClick={() => document.getElementById('live-layer')?.scrollIntoView({behavior:'smooth'})}><Navigation size={17}/> Explore live</button></div><div className="orbital"><div className="orbit o1"/><div className="orbit o2"/><div className="orbital-core"><Radio size={28}/></div></div></section><div id="live-layer"><Title eyebrow="LIVE LAYER" title="Right now"/><div className="moment-list">{loading ? <div className="loading">Loading Moments…</div> : moments.length ? moments.map(m => <MomentCard key={m.$id} m={m} profile={profiles[m.ownerId]} onJoin={onJoin} onReact={onReact} onComment={onComment}/>) : <div className="empty"><Radio size={34}/><h2>No live Moments yet.</h2><p>Be the first person to create one.</p><button className="primary" onClick={onCreate}><Plus size={17}/> Create Moment</button></div>}</div></div></div>
}

function Feed({moments, profiles, onJoin, onReact, onComment}) { return <div className="page"><Title eyebrow="LIVE FEED" title="Flow"/>{moments.length ? moments.map(m => <MomentCard key={m.$id} m={m} profile={profiles[m.ownerId]} onJoin={onJoin} onReact={onReact} onComment={onComment}/>) : <div className="empty"><Waves size={34}/><h2>Your Flow is waiting.</h2><p>New Moments will appear here.</p></div>}</div> }

function MomentCard({m, profile, onJoin, onReact, onComment}) {
  const [text, setText] = useState(''); const [watch,setWatch]=useState(false)
  return <article className="moment"><div className="moment-bg"/><div className="moment-shade"/>
    {m.imageUrl && <div className="moment-media">{m.imageUrl.startsWith('video::') ? <video src={m.imageUrl.slice(7)} controls playsInline preload="metadata"/> : m.imageUrl.startsWith('image::') ? <img src={m.imageUrl.slice(7)} alt="" /> : null}</div>}<div className="moment-top"><span className="live-pill">{m.category || 'NOW'}</span><span className="age">{timeAgo(m.$createdAt)}</span></div><div className="moment-bottom"><div className="moment-author"><div className="avatar">{(profile?.displayName||'W')[0].toUpperCase()}</div><span>{profile?.displayName||'WORLD User'} <small>@{profile?.username||'world'}</small></span></div><h2>{m.title}</h2><p>{m.description || 'Live Moment'}</p><div className="card-actions"><button onClick={() => setWatch(true)}><Radio size={14}/> Watch</button><button onClick={() => onJoin(m.$id)}><Users size={14}/> {m.participantsCount || 0} Join</button><button onClick={() => onReact(m.$id)}><Heart size={14}/> {m.reactionsCount || 0}</button></div><div className="comment-box"><input value={text} onChange={e => setText(e.target.value)} placeholder="Say something…"/><button onClick={() => { if (text.trim()) { onComment(m.$id, text.trim()); setText('') } }}><Send size={14}/></button></div></div>{watch&&<MomentWatch moment={m} profile={profile} onClose={()=>setWatch(false)}/>}</article>
}

function MomentWatch({moment,profile,onClose}){return <div className="modal-backdrop" onClick={onClose}><div className="sheet" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}><X size={19}/></button><div className="eyebrow">WATCH MOMENT</div><h2>{moment.title}</h2><p>{moment.description||'Live Moment'}</p><div className="watch-meta"><b>{profile?.displayName||'WORLD User'}</b><span>@{profile?.username||'world'}</span></div><div className="watch-stats"><span>{moment.participantsCount||0} joined</span><span>{moment.reactionsCount||0} reactions</span><span>{moment.commentsCount||0} comments</span></div><button className="primary wide" onClick={onClose}>Close</button></div></div>}

function NotificationsPanel({user,onClose}){const [items,setItems]=useState([]);useEffect(()=>{if(user)listNotifications(user.$id).then(setItems).catch(()=>setItems([]))},[user]);return <div className="modal-backdrop" onClick={onClose}><div className="sheet" onClick={e=>e.stopPropagation()}><button className="close" onClick={onClose}><X size={19}/></button><div className="eyebrow">ACTIVITY</div><h2>Notifications</h2>{!user?<div className="empty compact"><p>Sign in to see notifications.</p></div>:items.length?items.map((x,i)=><div className="activity-row" key={x.$id+'-'+i}><Bell size={15}/><div><b>{x._type==='joined'?'Someone joined your Moment':x._type==='reacted'?'Someone reacted to your Moment':'Someone commented on your Moment'}</b><small>{x.text||x.type||'New activity'} · {timeAgo(x.createdAt||x.$createdAt)}</small></div></div>):<div className="empty compact"><p>No new notifications yet.</p></div>}</div></div>}
...
function CreateMoment({user,onClose,onDone}) {
  const [type,setType]=useState('moment'),[title,setTitle]=useState(''),[description,setDescription]=useState(''),[category,setCategory]=useState('do'),[visibility,setVisibility]=useState('nearby'),[file,setFile]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState('');
  const submit=async e=>{e.preventDefault();setBusy(true);setError('');try{
    let mediaUrl='',mediaType='';
    if(file){mediaUrl=await uploadMedia(file,user.$id);mediaType=file.type.startsWith('video/')?'video':'image'}
    await createMoment(user.$id,{title,description,category:type==='article'?'article':category,visibility,mediaUrl,mediaType});
    await onDone()
  }catch(err){setError(err?.message||'Could not create Moment')}finally{setBusy(false)}};
  return <div className="modal-backdrop" onClick={onClose}><form className="sheet" onClick={e=>e.stopPropagation()} onSubmit={submit}>
    <button type="button" className="close" onClick={onClose}><X size={19}/></button><div className="eyebrow">CREATE</div><h2>{type==='article'?'Publish an article':'What is happening?'}</h2><p>{type==='article'?'Share a story with the WORLD.':'Share a temporary signal with the WORLD.'}</p>
    <div className="create-types"><button type="
